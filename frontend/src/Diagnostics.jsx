...
export function ScenarioPanel({ open, onClose }) {
  return <aside className={`scenario-panel ${open ? 'is-open' : ''}`} aria-hidden={!open}>
    ...
    <ScenarioGroup number="1" title="기본 구매 흐름" open><ol><li>기존 회원 계정으로 로그인</li><li>실제 상품 조회 및 선택</li><li>장바구니에 상품 추가</li><li>수량과 합계 금액 확인</li><li>Cart 상품으로 주문 생성</li><li>주문 상태와 상품 스냅샷 확인</li><li>실제 주문 금액으로 결제 승인</li><li>주문·결제 상태와 Cart 후처리 확인</li></ol></ScenarioGroup>
    ...
    <details className="known-issues"><summary>현재 확인 중인 이슈 <span>Known Issues</span></summary><ol><li><b>주문 생성 재고 중복 차감</b><small>실제 HTTP 재현 · 수량 2 주문에서 재고 4 감소</small></li><li><b>주문 취소 후 재고 불완전 복구</b><small>실제 HTTP 재현 · 생성 전 1000 → 생성 후 996 → 취소 후 998</small></li><li><b>주문 후 추가한 동일 상품 수량 삭제</b><small>실제 HTTP 재현 · 결제 승인 시 추가 수량을 포함한 CartItem 삭제</small></li><li><b>명시적 Payment FAIL HTTP 진입 경로 없음</b><small>현재 Frontend 테스트 불가</small></li></ol></details>
  </aside>;
}
...
export default function Diagnostics() {
  ...
  return <section className="http-history"><div className="section-heading"><div><p className="section-kicker">API ACTIVITY</p><h2>최근 HTTP 요청</h2></div><button className="button-text" onClick={() => setRequests([])}>기록 지우기</button></div><p className="muted">최근 30건 · 실제 Status, Request/Response, ErrorCode, 소요 시간을 기록하며 password와 JWT는 마스킹합니다.</p>
